...
export default function Navbar() {
  ...
  // 🟢 সরাসরি Navbar ফাইলের ভেতরেই সব ডাটা রাখা হলো
  const navLinks = [
    { name: "Home", href: "/" },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <nav className="hidden md:flex items-center gap-8">
              {navLinks.map((link) =>
                link.hasDropdown ? (
                  /* 🖼️ ইমেজ অনুযায়ী Exact Dropdown Component */
                  <div key={link.name} className="relative group py-6 cursor-pointer">
                    <div className="flex items-center gap-1.5 text-base font-semibold text-slate-700 group-hover:text-emerald-600 transition-colors">
                      <span>{link.name}</span>
                      <ChevronDown className="w-4 h-4 text-slate-500 group-hover:text-emerald-600 transition-transform group-hover:rotate-180" />
                    </div>

                    {/* Image Styled Floating Dropdown Card */}
                    <div className="absolute left-1/2 -translate-x-1/2 top-full hidden group-hover:block w-64 bg-white shadow-2xl rounded-2xl border border-slate-100 p-3 z-50 transition-all duration-200 animate-in fade-in slide-in-from-top-2">
                      <div className="flex flex-col gap-1">
                        {link.subItems?.map((subItem) => (
                          <Link
                            key={subItem.name}
                            href={subItem.href}
                            className="px-4 py-2.5 text-sm font-semibold text-slate-600 hover:text-emerald-600 hover:bg-slate-50 rounded-xl transition-all duration-150"
                          >
                            {subItem.name}
                          </Link>
                        ))}
