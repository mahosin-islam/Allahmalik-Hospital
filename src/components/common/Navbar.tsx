          ...
          <div className="flex items-center gap-2 overflow-hidden w-full md:w-1/2">
            ...
            <div className="overflow-hidden whitespace-nowrap w-full">
              <div className="inline-block animate-marquee pl-full text-[11px] sm:text-xs">
                জরুরি প্রয়োজনে ২৪ ঘণ্টা ডাক্তার ও ডায়াগনস্টিক সেবা পাবেন। হটলাইন: 01965-331717।
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <nav className="hidden md:flex items-center gap-1.5 lg:gap-3">
              {navLinks.map((link) => {
                if (link.hasDropdown) {
                  const isParentActive = link.subItems?.some(
                  ...
                  return (
                    <div key={link.name} className="relative group py-6 cursor-pointer">
                      <div
                        className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm lg:text-base transition-all duration-200 ${
                          isParentActive
                            ? "bg-emerald-50 text-emerald-700 font-bold border border-emerald-200/60 shadow-sm"
                            : "font-semibold text-slate-700 hover:text-emerald-600 hover:bg-slate-50"
                        }`}
                      >
                        <span>{link.name}</span>
                      ...
                      <div className="absolute left-1/2 -translate-x-1/2 top-full hidden group-hover:block w-60 lg:w-64 bg-white shadow-2xl rounded-2xl border border-slate-100 p-2 z-50 transition-all duration-200 animate-in fade-in slide-in-from-top-2">
                          ...
                          {link.subItems?.map((subItem) => {
                            ...
                            return (
                              <Link
                                key={subItem.name}
                                href={subItem.href}
                                className={`px-3.5 py-2 text-xs lg:text-sm rounded-xl transition-all duration-150 ${
                                  isSubActive
                                    ? "bg-emerald-50 text-emerald-700 font-bold border border-emerald-200/40"
                                    : "font-semibold text-slate-600 hover:text-emerald-600 hover:bg-slate-50"
                                }`}
                              >
                                {subItem.name}
                              </Link>
                ...
                return (
                  <Link
                    key={link.name}
                    href={link.href!}
                    className={`px-3.5 py-2 text-sm lg:text-base rounded-xl whitespace-nowrap transition-all duration-200 ${
                      isActive
                        ? "bg-emerald-50 text-emerald-700 font-bold border border-emerald-200/60 shadow-sm"
                        : "font-semibold text-slate-700 hover:text-emerald-600 hover:bg-slate-50"
                    }`}
            ...
            <div className="hidden md:flex items-center shrink-0">
              ...
              >
                ...
                <span className="hidden lg:inline">Emergency: +880 1965-331717</span>
            ...
            {navLinks.map((link) => {
                ...
                return (
                  <div key={link.name}>
                    <button
                      ...
                      className={`flex items-center justify-between w-full px-3 py-2 text-base rounded-xl transition-all ${
                        isParentActive
                          ? "bg-emerald-50 text-emerald-700 font-bold"
                          : "font-semibold text-slate-700 hover:text-emerald-600"
                      }`}
                    >
                      <span>{link.name}</span>
                      <ChevronDown
                    ...
                    {mobileDropdownOpen === link.name && (
                      <div className="pl-4 py-1 space-y-1 border-l-2 border-emerald-200 my-1 ml-2">
                        {link.subItems?.map((subItem) => {
                          const isSubActive = pathname === subItem.href;
                          return (
                            <Link
              ...
              return (
                <Link
                  ...
                  href={link.href!}
                  onClick={() => setIsOpen(false)}
                  className={`block px-3 py-2 text-base rounded-xl transition-all ${
                    isActive
